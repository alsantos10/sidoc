						...
						<table class="table table-striped">
							...
							<jsp:useBean id="dao" class="br.com.sidoc.DAO.UsuarioDAO">
								<c:forEach var="funci" items="${dao.lista}">
									<c:if
										test="${usuarioLogado.id != funci.id && (funci.gerente.id>0 && usuarioLogado.id==funci.gerente.id)}">
										<tr class="text text-${funci.ativo=='s'?'default':'danger'}">
											...
											<td><c:url
													...
													value="/sistema?c=Funcionario&acao=senha&id=${funci.id}"
													var="linkSenha" /> <c:url
													value="/sistema?c=Funcionario&acao=excluir&id=${funci.id}"
													var="linkExcluir" /> <a href="${linkSenha}"><i
													class="fa fa-unlock-alt fa-lg"></i></a> <a
												href="${linkAlterar}"><i
													class="fa fa-pencil-square-o fa-lg icon-fixed-width icon-pencil"></i></a>
												<a href="${linkExcluir}" class="btnExcluir"><i
													class="fa fa-minus-square-o fa-lg text-danger"></i></a></td>
										</tr>
									</c:if>
